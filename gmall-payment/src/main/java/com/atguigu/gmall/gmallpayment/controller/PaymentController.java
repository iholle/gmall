package com.atguigu.gmall.gmallpayment.controller;
...
import com.atguigu.gmall.gmallpayment.config.AlipayConfig;
...
public class PaymentController {
    ...
    public String submit(String orderId, HttpServletResponse response){
        ...
        alipayRequest.setReturnUrl(AlipayConfig.return_payment_url);//返回路径
        alipayRequest.setNotifyUrl(AlipayConfig.notify_payment_url);//回调路径

        long currentTimeMillis = System.currentTimeMillis();
        String outTradeNo = "Atguigugmall-"+orderId+"-"+currentTimeMillis;
        BigDecimal totalAmount = orderInfo.getTotalAmount();
        String productNo = "FAST_INSTANT_TRADE_PAY";
        String subject = orderInfo.genSubject();//订单主题
            ...
            if("TRADE_SUCCESS".equals(tradeStatus)){
                ...
                if(paymentInfo.getTotalAmount().compareTo(new BigDecimal(total_amount))==0){
                    if(paymentInfo.getPaymentStatus().equals(PaymentStatus.UNPAID)){
                        ...
                        paymentService.updatePaymentInfoByOutTradeNo(out_trade_no,paymentInfoForUpdate);
                        //发送异步消息给订单
                        return "success";
    ...
    public String refund(String orderId) throws AlipayApiException {
        ...
        PaymentInfo paymentInfo = paymentService.getPaymentInfo(paymentInfoQuery);//查询需要退款的订单信息
        ...
        if(response.isSuccess()){
            ...
            paymentService.updatePaymentInfoByOutTradeNo(paymentInfo.getOutTradeNo(),paymentInfoForUpdate);
            //处理订单状态
            //异步处理
            return "success";
        }else {
